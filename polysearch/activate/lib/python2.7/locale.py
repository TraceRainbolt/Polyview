***REMOVED*** Locale support.
    ...
    setlocale(***REMOVED*** function. It also includes default encodings for all
...
__all__ = ["getlocale", "getdefaultlocale", "getpreferredencoding", "Error",
           ...
           "LC_NUMERIC", "LC_ALL", "CHAR_MAX"***REMOVED***
...
try:
    ...
    def localeconv(***REMOVED***:
        ***REMOVED*** localeconv(***REMOVED*** -> dict.
            Returns numeric and monetary locale-specific parameters.
        ***REMOVED***
        # 'C' locale default values
        return {'grouping': [127***REMOVED***,
                'currency_symbol': '',
                'n_sign_posn': 127,
                'p_cs_precedes': 127,
                'n_cs_precedes': 127,
                'mon_grouping': [***REMOVED***,
                'n_sep_by_space': 127,
                'decimal_point': '.',
                'negative_sign': '',
                'positive_sign': '',
                ...
                'frac_digits': 127,
                'mon_decimal_point': '',
                'int_frac_digits': 127***REMOVED***

    def setlocale(category, value=None***REMOVED***:
        ***REMOVED*** setlocale(integer,string=None***REMOVED*** -> string.
            Activates/queries locale processing.
        ***REMOVED***
        if value not in (None, '', 'C'***REMOVED***:
            raise Error, '_locale emulation only supports "C" locale'
        return 'C'

    def strcoll(a,b***REMOVED***:
        ***REMOVED*** strcoll(string,string***REMOVED*** -> int.
            Compares two strings according to the locale.
        ***REMOVED***
        return cmp(a,b***REMOVED***

    def strxfrm(s***REMOVED***:
        ***REMOVED*** strxfrm(string***REMOVED*** -> string.
...
_override_localeconv = {***REMOVED***

@functools.wraps(_localeconv***REMOVED***
def localeconv(***REMOVED***:
    d = _localeconv(***REMOVED***
    if _override_localeconv:
        d.update(_override_localeconv***REMOVED***
    return d
...
def _grouping_intervals(grouping***REMOVED***:
    last_interval = None
    for interval in grouping:
        ...
        if interval == 0:
            if last_interval is None:
                raise ValueError("invalid grouping"***REMOVED***
            while True:
                yield last_interval
        yield interval
...
def _group(s, monetary=False***REMOVED***:
    conv = localeconv(***REMOVED***
    thousands_sep = conv[monetary and 'mon_thousands_sep' or 'thousands_sep'***REMOVED***
    grouping = conv[monetary and 'mon_grouping' or 'grouping'***REMOVED***
    if not grouping:
        return (s, 0***REMOVED***
    if s[-1***REMOVED*** == ' ':
        stripped = s.rstrip(***REMOVED***
        right_spaces = s[len(stripped***REMOVED***:***REMOVED***
        s = stripped
    else:
        right_spaces = ''
    left_spaces = ''
    groups = [***REMOVED***
    for interval in _grouping_intervals(grouping***REMOVED***:
        if not s or s[-1***REMOVED*** not in "0123456789":
            # only non-digit characters remain (sign, spaces***REMOVED***
            left_spaces = s
            s = ''
            break
        groups.append(s[-interval:***REMOVED******REMOVED***
        s = s[:-interval***REMOVED***
    if s:
        groups.append(s***REMOVED***
    groups.reverse(***REMOVED***
    return (
        left_spaces + thousands_sep.join(groups***REMOVED*** + right_spaces,
        len(thousands_sep***REMOVED*** * (len(groups***REMOVED*** - 1***REMOVED***
    ***REMOVED***

# Strip a given amount of excess padding from the given string
def _strip_padding(s, amount***REMOVED***:
    lpos = 0
    while amount and s[lpos***REMOVED*** == ' ':
        lpos += 1
        amount -= 1
    rpos = len(s***REMOVED*** - 1
    while amount and s[rpos***REMOVED*** == ' ':
        rpos -= 1
        amount -= 1
    return s[lpos:rpos+1***REMOVED***

_percent_re = re.compile(r'%(?:\((?P<key>.*?***REMOVED***\***REMOVED******REMOVED***?'
                         r'(?P<modifiers>[-#0-9 +*.hlL***REMOVED****?***REMOVED***[eEfFgGdiouxXcrs%***REMOVED***'***REMOVED***

def format(percent, value, grouping=False, monetary=False, *additional***REMOVED***:
    ***REMOVED***Returns the locale-aware substitution of a %? specifier
    (percent***REMOVED***.

    additional is for format strings which contain one or more
    '*' modifiers.***REMOVED***
    # this is only for one-percent-specifier strings and this should be checked
    match = _percent_re.match(percent***REMOVED***
    if not match or len(match.group(***REMOVED******REMOVED***!= len(percent***REMOVED***:
        raise ValueError(("format(***REMOVED*** must be given exactly one %%char "
                         "format specifier, %s not valid"***REMOVED*** % repr(percent***REMOVED******REMOVED***
    return _format(percent, value, grouping, monetary, *additional***REMOVED***

def _format(percent, value, grouping=False, monetary=False, *additional***REMOVED***:
    if additional:
        formatted = percent % ((value,***REMOVED*** + additional***REMOVED***
    else:
        formatted = percent % value
    # floats and decimal ints need special action!
    if percent[-1***REMOVED*** in 'eEfFgG':
        seps = 0
        parts = formatted.split('.'***REMOVED***
        if grouping:
            parts[0***REMOVED***, seps = _group(parts[0***REMOVED***, monetary=monetary***REMOVED***
        decimal_point = localeconv(***REMOVED***[monetary and 'mon_decimal_point'
                                              or 'decimal_point'***REMOVED***
        formatted = decimal_point.join(parts***REMOVED***
        if seps:
            formatted = _strip_padding(formatted, seps***REMOVED***
    elif percent[-1***REMOVED*** in 'diu':
        seps = 0
        if grouping:
            formatted, seps = _group(formatted, monetary=monetary***REMOVED***
        if seps:
            formatted = _strip_padding(formatted, seps***REMOVED***
    return formatted

def format_string(f, val, grouping=False***REMOVED***:
    ***REMOVED***Formats a string in the same way that the % formatting would use,
    but takes the current locale into account.
    Grouping is applied if the third parameter is true.***REMOVED***
    percents = list(_percent_re.finditer(f***REMOVED******REMOVED***
    new_f = _percent_re.sub('%s', f***REMOVED***

    if operator.isMappingType(val***REMOVED***:
        new_val = [***REMOVED***
        for perc in percents:
            if perc.group(***REMOVED***[-1***REMOVED***=='%':
                new_val.append('%'***REMOVED***
            else:
                new_val.append(format(perc.group(***REMOVED***, val, grouping***REMOVED******REMOVED***
    else:
        ...
        for perc in percents:
            if perc.group(***REMOVED***[-1***REMOVED***=='%':
                ...
                starcount = perc.group('modifiers'***REMOVED***.count('*'***REMOVED***
                new_val.append(_format(perc.group(***REMOVED***,
                                      val[i***REMOVED***,
                                      grouping,
                                      False,
                                      *val[i+1:i+1+starcount***REMOVED******REMOVED******REMOVED***
                i += (1 + starcount***REMOVED***
    val = tuple(new_val***REMOVED***

    return new_f % val

def currency(val, symbol=True, grouping=False, international=False***REMOVED***:
    ***REMOVED***Formats val according to the currency settings
    in the current locale.***REMOVED***
    conv = localeconv(***REMOVED***

    # check for illegal values
    digits = conv[international and 'int_frac_digits' or 'frac_digits'***REMOVED***
    if digits == 127:
        raise ValueError("Currency formatting is not possible using "
                         "the 'C' locale."***REMOVED***

    s = format('%%.%if' % digits, abs(val***REMOVED***, grouping, monetary=True***REMOVED***
    # '<' and '>' are markers if the sign must be inserted between symbol and value
    s = '<' + s + '>'

    if symbol:
        smb = conv[international and 'int_curr_symbol' or 'currency_symbol'***REMOVED***
        precedes = conv[val<0 and 'n_cs_precedes' or 'p_cs_precedes'***REMOVED***
        separated = conv[val<0 and 'n_sep_by_space' or 'p_sep_by_space'***REMOVED***

        if precedes:
            s = smb + (separated and ' ' or ''***REMOVED*** + s
        else:
            s = s + (separated and ' ' or ''***REMOVED*** + smb

    sign_pos = conv[val<0 and 'n_sign_posn' or 'p_sign_posn'***REMOVED***
    sign = conv[val<0 and 'negative_sign' or 'positive_sign'***REMOVED***
    ...
    return s.replace('<', ''***REMOVED***.replace('>', ''***REMOVED***

def str(val***REMOVED***:
    ***REMOVED***Convert float to integer, taking the locale into account.***REMOVED***
    return format("%.12g", val***REMOVED***

def atof(string, func=float***REMOVED***:
    "Parses a string as a float according to the locale settings."
    #First, get rid of the grouping
    ts = localeconv(***REMOVED***['thousands_sep'***REMOVED***
    if ts:
        string = string.replace(ts, ''***REMOVED***
    #next, replace the decimal point with a dot
    dd = localeconv(***REMOVED***['decimal_point'***REMOVED***
    if dd:
        string = string.replace(dd, '.'***REMOVED***
    #finally, parse the string
    return func(string***REMOVED***

def atoi(str***REMOVED***:
    "Converts a string to an integer according to the locale settings."
    return atof(str, int***REMOVED***

def _test(***REMOVED***:
    setlocale(LC_ALL, ""***REMOVED***
    #do grouping
    s1 = format("%d", 123456789,1***REMOVED***
    print s1, "is", atoi(s1***REMOVED***
    #standard formatting
    s1 = str(3.14***REMOVED***
    print s1, "is", atof(s1***REMOVED***
...
# overridden below***REMOVED***
_setlocale = setlocale

# Avoid relying on the locale-dependent .lower(***REMOVED*** method
# (see issue #1813***REMOVED***.
_ascii_lower_map = ''.join(
    chr(x + 32 if x >= ord('A'***REMOVED*** and x <= ord('Z'***REMOVED*** else x***REMOVED***
    for x in range(256***REMOVED***
***REMOVED***

def _replace_encoding(code, encoding***REMOVED***:
    if '.' in code:
        langname = code[:code.index('.'***REMOVED******REMOVED***
    else:
        langname = code
    # Convert the encoding to a C lib compatible encoding string
    norm_encoding = encodings.normalize_encoding(encoding***REMOVED***
    #print('norm encoding: %r' % norm_encoding***REMOVED***
    norm_encoding = encodings.aliases.aliases.get(norm_encoding,
                                                  norm_encoding***REMOVED***
    #print('aliased encoding: %r' % norm_encoding***REMOVED***
    encoding = locale_encoding_alias.get(norm_encoding,
                                         norm_encoding***REMOVED***
    #print('found encoding %r' % encoding***REMOVED***
    return langname + '.' + encoding

def normalize(localename***REMOVED***:

    ***REMOVED*** Returns a normalized locale code for the given locale
        name.
        ...
        the default encoding for the locale code just like setlocale(***REMOVED***
        does.

    ***REMOVED***
    # Normalize the locale name and extract the encoding and modifier
    if isinstance(localename, _unicode***REMOVED***:
        localename = localename.encode('ascii'***REMOVED***
    code = localename.translate(_ascii_lower_map***REMOVED***
    if ':' in code:
        # ':' is sometimes used as encoding delimiter.
        code = code.replace(':', '.'***REMOVED***
    if '@' in code:
        code, modifier = code.split('@', 1***REMOVED***
    else:
        modifier = ''
    if '.' in code:
        langname, encoding = code.split('.'***REMOVED***[:2***REMOVED***
    else:
        langname = code
        encoding = ''

    # First lookup: fullname (possibly with encoding and modifier***REMOVED***
    lang_enc = langname
    if encoding:
        norm_encoding = encoding.replace('-', ''***REMOVED***
        norm_encoding = norm_encoding.replace('_', ''***REMOVED***
        lang_enc += '.' + norm_encoding
    lookup_name = lang_enc
    if modifier:
        lookup_name += '@' + modifier
    code = locale_alias.get(lookup_name, None***REMOVED***
    if code is not None:
        return code
    #print('first lookup failed'***REMOVED***

    if modifier:
        # Second try: fullname without modifier (possibly with encoding***REMOVED***
        code = locale_alias.get(lang_enc, None***REMOVED***
        if code is not None:
            #print('lookup without modifier succeeded'***REMOVED***
            if '@' not in code:
                return code + '@' + modifier
            if code.split('@', 1***REMOVED***[1***REMOVED***.translate(_ascii_lower_map***REMOVED*** == modifier:
                return code
        #print('second lookup failed'***REMOVED***

    if encoding:
        # Third try: langname (without encoding, possibly with modifier***REMOVED***
        lookup_name = langname
        if modifier:
            lookup_name += '@' + modifier
        code = locale_alias.get(lookup_name, None***REMOVED***
        if code is not None:
            #print('lookup without encoding succeeded'***REMOVED***
            if '@' not in code:
                return _replace_encoding(code, encoding***REMOVED***
            code, modifier = code.split('@', 1***REMOVED***
            return _replace_encoding(code, encoding***REMOVED*** + '@' + modifier

        if modifier:
            # Fourth try: langname (without encoding and modifier***REMOVED***
            code = locale_alias.get(langname, None***REMOVED***
            if code is not None:
                #print('lookup without modifier and encoding succeeded'***REMOVED***
                if '@' not in code:
                    return _replace_encoding(code, encoding***REMOVED*** + '@' + modifier
                code, defmod = code.split('@', 1***REMOVED***
                if defmod.translate(_ascii_lower_map***REMOVED*** == modifier:
                    return _replace_encoding(code, encoding***REMOVED*** + '@' + defmod

    return localename

def _parse_localename(localename***REMOVED***:

    ***REMOVED*** Parses the locale code for localename and returns the
        result as tuple (language code, encoding***REMOVED***.
    ...
    ***REMOVED***
    code = normalize(localename***REMOVED***
    if '@' in code:
    ...
    if '.' in code:
        return tuple(code.split('.'***REMOVED***[:2***REMOVED******REMOVED***
    elif code == 'C':
        return None, None
    raise ValueError, 'unknown locale: %s' % localename

def _build_localename(localetuple***REMOVED***:

    ***REMOVED*** Builds a locale code from the given tuple (language code,
        encoding***REMOVED***.
...
def getdefaultlocale(envvars=('LC_ALL', 'LC_CTYPE', 'LANG', 'LANGUAGE'***REMOVED******REMOVED***:

    ***REMOVED*** Tries to determine the default locale settings and returns
        them as tuple (language code, encoding***REMOVED***.

        According to POSIX, a program which has not called
        setlocale(LC_ALL, ""***REMOVED*** runs using the portable 'C' locale.
        Calling setlocale(LC_ALL, ""***REMOVED*** lets it use the default locale as
    ...
    ***REMOVED***
    ...
    try:
        ...
        import _locale
        code, encoding = _locale._getdefaultlocale(***REMOVED***
    except (ImportError, AttributeError***REMOVED***:
        pass
    else:
        # make sure the code/encoding values are valid
        if sys.platform == "win32" and code and code[:2***REMOVED*** == "0x":
            # map windows language identifier to language name
            code = windows_locale.get(int(code, 0***REMOVED******REMOVED***
    ...
    ***REMOVED***
    ...
    for variable in envvars:
        localename = lookup(variable,None***REMOVED***
        if localename:
            if variable == 'LANGUAGE':
                localename = localename.split(':'***REMOVED***[0***REMOVED***
            break
    else:
        localename = 'C'
    return _parse_localename(localename***REMOVED***


def getlocale(category=LC_CTYPE***REMOVED***:

    ***REMOVED*** Returns the current setting for the given locale category as
        tuple (language code, encoding***REMOVED***.
    ...
    ***REMOVED***
    localename = _setlocale(category***REMOVED***
    if category == LC_ALL and ';' in localename:
        raise TypeError, 'category LC_ALL is not supported'
    return _parse_localename(localename***REMOVED***

def setlocale(category, locale=None***REMOVED***:

    ***REMOVED*** Set the locale for the given category.  The locale can be
        a string, an iterable of two strings (language code and encoding***REMOVED***,
        or None.
    ...
    ***REMOVED***
    if locale and type(locale***REMOVED*** is not type(""***REMOVED***:
        # convert to string
        locale = normalize(_build_localename(locale***REMOVED******REMOVED***
    return _setlocale(category, locale***REMOVED***

def resetlocale(category=LC_ALL***REMOVED***:

    ***REMOVED*** Sets the locale for category to the default setting.

        The default setting is determined by calling
        getdefaultlocale(***REMOVED***. category defaults to LC_ALL.

    ***REMOVED***
    _setlocale(category, _build_localename(getdefaultlocale(***REMOVED******REMOVED******REMOVED***

if sys.platform.startswith("win"***REMOVED***:
    # On Win32, this will return the ANSI code page
    def getpreferredencoding(do_setlocale = True***REMOVED***:
        ***REMOVED***Return the charset that the user is likely using.***REMOVED***
        import _locale
        return _locale._getdefaultlocale(***REMOVED***[1***REMOVED***
else:
    ...
    try:
        ...
        def getpreferredencoding(do_setlocale = True***REMOVED***:
            ***REMOVED***Return the charset that the user is likely using,
            by looking at environment variables.***REMOVED***
            return getdefaultlocale(***REMOVED***[1***REMOVED***
    else:
        def getpreferredencoding(do_setlocale = True***REMOVED***:
            ***REMOVED***Return the charset that the user is likely using,
            according to the system configuration.***REMOVED***
            if do_setlocale:
                oldloc = setlocale(LC_CTYPE***REMOVED***
                try:
                    setlocale(LC_CTYPE, ""***REMOVED***
                except Error:
                    pass
                result = nl_langinfo(CODESET***REMOVED***
                setlocale(LC_CTYPE, oldloc***REMOVED***
                return result
            else:
                return nl_langinfo(CODESET***REMOVED***
...
# to C locale encoding names (case-sensitive***REMOVED***. Note that normalize(***REMOVED***
...
# (case-sensitive***REMOVED***. Encodings are always separated from the locale
# name using a dot ('.'***REMOVED***; they should only be given in case the
# language name is needed to interpret the given encoding alias
# correctly (CJK codes often have this need***REMOVED***.
#
# Note that the normalize(***REMOVED*** function which uses this tables
...
#    0x0455: "my_MM", # Burmese - Not supported
    ...
    0x046b: "quz_BO",# Quechua (Bolivia***REMOVED***
    0x086b: "quz_EC",# Quechua (Ecuador***REMOVED***
    0x0c6b: "quz_PE",# Quechua (Peru***REMOVED***
    ...
    0x0c0a: "es_ES", # Spanish - Spain (Modern***REMOVED***
...
def _print_locale(***REMOVED***:

    ***REMOVED*** Test function.
    ***REMOVED***
    categories = {***REMOVED***
    def _init_categories(categories=categories***REMOVED***:
        for k,v in globals(***REMOVED***.items(***REMOVED***:
            if k[:3***REMOVED*** == 'LC_':
                categories[k***REMOVED*** = v
    _init_categories(***REMOVED***
    del categories['LC_ALL'***REMOVED***

    print 'Locale defaults as determined by getdefaultlocale(***REMOVED***:'
    print '-'*72
    lang, enc = getdefaultlocale(***REMOVED***
    print 'Language: ', lang or '(undefined***REMOVED***'
    print 'Encoding: ', enc or '(undefined***REMOVED***'
    print

    print 'Locale settings on startup:'
    print '-'*72
    for name,category in categories.items(***REMOVED***:
        print name, '...'
        lang, enc = getlocale(category***REMOVED***
        print '   Language: ', lang or '(undefined***REMOVED***'
        print '   Encoding: ', enc or '(undefined***REMOVED***'
        print

    print
    print 'Locale settings after calling resetlocale(***REMOVED***:'
    print '-'*72
    resetlocale(***REMOVED***
    for name,category in categories.items(***REMOVED***:
        print name, '...'
    ...
    try:
        ...
        print 'NOTE:'
        print 'setlocale(LC_ALL, ""***REMOVED*** does not support the default locale'
        print 'given in the OS environment variables.'
    else:
        print
        print 'Locale settings after calling setlocale(LC_ALL, ""***REMOVED***:'
        print '-'*72
        for name,category in categories.items(***REMOVED***:
            print name, '...'
            lang, enc = getlocale(category***REMOVED***
...
try:
    ...
    __all__.append("LC_MESSAGES"***REMOVED***

if __name__=='__main__':
    print 'Locale aliasing:'
    print
    _print_locale(***REMOVED***
    print
